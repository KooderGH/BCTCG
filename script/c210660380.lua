...
function s.initial_effect(c)
    ...
    c:RegisterEffect(e2)
    --If you control "D'arktanyan", you can activate this effect (Quick): Destroy all other cards on the field. (3)
    local e3=Effect.CreateEffect(c)
    e3:SetDescription(aux.Stringid(id,0))
    e3:SetCategory(CATEGORY_DESTROY)
    e3:SetProperty(EFFECT_FLAG_NO_TURN_RESET+EFFECT_FLAG_DAMAGE_STEP)
...
--e3
function s.darktfilter(c)
	return c:IsCode(210660441) and c:IsFaceup()
end
function s.descon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(s.darktfilter,tp,LOCATION_MZONE,0,1,nil)
end
function s.desfilter(c)
	return not c:IsCode(210660441)
end
