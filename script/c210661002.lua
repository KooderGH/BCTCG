...
function s.initial_effect(c)
	...
	e19:SetDescription(aux.Stringid(id,4))
...
function s.sstpop2(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>0
	    ...
	    --Cannot be targeted by effects (1)
		local e2=Effect.CreateEffect(e:GetHandler())
		e2:SetType(EFFECT_TYPE_SINGLE)
	    ...
	    e3:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DAMAGE_CAL+EFFECT_FLAG_DELAY)
	    e3:SetRange(LOCATION_MZONE)
		e3:SetCountLimit(1,{id,3},EFFECT_COUNT_CODE_OATH)
	    e3:SetTarget(s.spelltarget)
	    e3:SetOperation(s.spellop)
	    ...
	    e4:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_DAMAGE_CAL+EFFECT_FLAG_DELAY)
	    e4:SetRange(LOCATION_SZONE)
		e4:SetCountLimit(1,{id,3},EFFECT_COUNT_CODE_OATH)
	    e4:SetTarget(s.spelltarget2)
	    e4:SetOperation(s.spellop2)
...
--Special Summon Thermae K41 function
s.listed_names={210668003}
function s.ssttg3(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
	end
	Duel.SetOperationInfo(0,CATEGORY_TOKEN,nil,1,tp,0)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,0)
end
function s.sstpop3(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,210668003,0,TYPES_TOKEN,500,1500,2,RACE_MACHINE,ATTRIBUTE_DARK) then
		local c=e:GetHandler()
		local e1=Effect.CreateEffect(c)
		e1:SetType(EFFECT_TYPE_SINGLE)
		e1:SetCode(EFFECT_UPDATE_DEFENSE)
		...
		e1:SetReset(RESET_EVENT+RESETS_STANDARD_DISABLE)
		c:RegisterEffect(e1)
		local token2=Duel.CreateToken(tp,210668003)
		Duel.SpecialSummonStep(token2,0,tp,tp,false,false,POS_FACEUP)
	    --Cannot be targeted by effects (1)
		local e2=Effect.CreateEffect(e:GetHandler())
		e2:SetType(EFFECT_TYPE_SINGLE)
