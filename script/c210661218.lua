...
--Scripted by Kooder
--Effects:
-- When this card is Summoned; Add 1 "MARCO", "TARMA", or "ERI" from your Deck or GY to your hand.
...
-- * Special Summon up to 2 "MARCO", "TARMA", "ERI", or "FIO" from your Hand.
-- * Send 1 monster from your opponent side of the field to the GY.
local s,id=GetID()
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	if e:GetLabel()==1 then
		-- SP Summon up to 2 MTEF
		if Duel.GetLocationCount(tp,LOCATION_MZONE)<2 then return end
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
			local g=Duel.SelectMatchingCard(tp,s.mteffilter,tp,LOCATION_HAND,0,1,2,nil,e,tp)
			if #g>0 then
				Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)
			end
	else
		-- 1 Monster card to GY
